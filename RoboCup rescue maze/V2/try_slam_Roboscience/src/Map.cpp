    ...
    for (auto& pt : points) {
        ...
        for (size_t k = 0; k + 1 < ray.size(); ++k) {
            ...
            if (cx >= 0 && cy >= 0 && cx < width && cy < height) {
                ...
                if (grid[cx][cy] == Map::UNKNOWN) grid[cx][cy] = Map::FREE;
            }
        }
        // endpoint
        std::cout<<" rayx: "<<ray.back().first<<" rayy: "<<ray.back().second<<std::endl;
...
void Map::debugPrintMap(const Eigen::Vector3d& robotPose) const {
    ...
    for (int y = height - 1; y >= 0; --y) {
        for (int x = 0; x < width; ++x) {
            int cell = grid[x][y];
            if(rx==x&&ry==y)std::cout<<"*";
            else if (cell == Map::OCCUPIED) std::cout << "#";
            else if (abs(rx-x)<=2&&abs(ry-y)<=2) std::cout << "*";
